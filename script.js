...
function calculateRecommendedPurchasePrice() {
    ...
    const targetProfitRate = parseFloat(document.getElementById('targetProfitRate').value) / 100;
    ...
    if (productName && storage && recommendedPrices[productName] && recommendedPrices[productName][storage]) {
        ...
        const commission = Math.round(recommendedPrice * commissionRate);
        const profit = Math.round(recommendedPrice * targetProfitRate);

        // 推奨仕入れ値を計算
        const maxPurchasePrice = Math.floor(recommendedPrice - commission - shippingCost - profit - totalPartsCost);
...
document.getElementById('shippingCost').addEventListener('change', calculateRecommendedPurchasePrice);
document.getElementById('commissionRate').addEventListener('input', calculateRecommendedPurchasePrice);
document.getElementById('targetProfitRate').addEventListener('input', calculateRecommendedPurchasePrice);